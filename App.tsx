...
import { generatePainting } from './geminiService';
...
type PaintingStyle = 'watercolor' | 'oil';
...
const App: React.FC = () => {
    ...
    const [generatingStyle, setGeneratingStyle] = useState<PaintingStyle | null>(null);
    const [generatedPainting, setGeneratedPainting] = useState<string>('');
    const [activePaintingStyle, setActivePaintingStyle] = useState<PaintingStyle | null>(null);
    ...
    const handleGeneratePainting = async (style: PaintingStyle) => {
        ...
        try {
            ...
            // If a painting already exists, we can re-use the captured map image
            if (capturedMapImage) {
            ...
            const paintingDataUrl = await generatePainting(imageToProcess, style);
            setGeneratedPainting(paintingDataUrl);
            setActivePaintingStyle(style);
        } catch (err) {
            setError(err instanceof Error ? err.message : "Failed to generate painting.");
            setGeneratedPainting('');
            setCapturedMapImage('');
            setActivePaintingStyle(null);
        } finally {
    ...
    const handleBackToMap = () => {
        setGeneratedPainting('');
        // Keep capturedMapImage so user doesn't have to recapture
        setActivePaintingStyle(null);
    };

    const handleDownloadPainting = () => {
        if (!generatedPainting) return;
        const link = document.createElement('a');
        link.href = generatedPainting;
        const safeAddress = address.replace(/[^a-z0-9]/gi, '_').toLowerCase();
        link.download = `${activePaintingStyle}_${safeAddress || 'painting'}.png`;
        ...
        <div className="container mx-auto p-4 md:p-8 max-w-4xl">
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 md:p-8">
                <header>
                    ...
                    <p className="text-gray-600 mb-6">Enter an address, then turn the satellite image into a beautiful painting.</p>
                </header>
                ...
                <div className="flex flex-wrap gap-4 mb-4">
                    {mapInitialized && (
                        <>
                            <button
                                onClick={() => handleGeneratePainting('watercolor')}
                                disabled={isGeneratingPainting}
                                className="flex-grow bg-green-600 text-white font-semibold py-3 px-6 rounded-lg hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-transform transform hover:scale-105 shadow-md disabled:bg-gray-400 disabled:cursor-not-allowed flex items-center justify-center h-12 whitespace-nowrap"
                            >
                                {isGeneratingPainting && generatingStyle === 'watercolor' ? (
                                    <svg className="animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                                ) : (
                                    generatedPainting ? '🎨 Re-create Watercolor' : '🎨 Create Watercolor'
                                )}
                            </button>
                             <button
                                onClick={() => handleGeneratePainting('oil')}
                                disabled={isGeneratingPainting}
                                className="flex-grow bg-purple-600 text-white font-semibold py-3 px-6 rounded-lg hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 transition-transform transform hover:scale-105 shadow-md disabled:bg-gray-400 disabled:cursor-not-allowed flex items-center justify-center h-12 whitespace-nowrap"
                            >
                                {isGeneratingPainting && generatingStyle === 'oil' ? (
                                    <svg className="animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                                ) : (
                                    generatedPainting ? '🖌️ Re-create Oil Painting' : '🖌️ Create Oil Painting'
                                )}
                            </button>
                        </>
                    )}
                     {generatedPainting && !isGeneratingPainting && (
                        <button
                ...
                <div className="bg-gray-50 rounded-2xl h-[70vh] shadow-inner overflow-hidden relative">
                    <div ref={mapRef} className={`w-full h-full rounded-2xl transition-opacity duration-300 ${mapInitialized && !generatedPainting ? 'opacity-100' : 'opacity-0'}`} />
                    ...
                    {generatedPainting && !isGeneratingPainting && (
                        <div className="absolute inset-0 w-full h-full flex flex-col bg-white rounded-2xl z-20 transition-opacity duration-300">
                            <div className="flex justify-between items-center p-4 bg-white border-b border-gray-200">
                                <div>
                                    <h3 className="text-lg font-semibold text-gray-900">Generated {activePaintingStyle === 'watercolor' ? 'Watercolor' : 'Oil'} Painting</h3>
                            ...
                            <div className="flex flex-col flex-row text-center items-center justify-center p-4 bg-gray-50 min-h-0">
                                <img 
                                    src={generatedPainting} 
                                    alt={`${activePaintingStyle === 'watercolor' ? 'Watercolor' : 'Oil'} painting of the building`}
