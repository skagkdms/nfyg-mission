...
/**
 * Generates a painting from a satellite image in a specified style.
 * @param imageDataUrl A data URL string of the source image (e.g., 'data:image/png;base64,...').
 * @param style The desired painting style, either 'watercolor' or 'oil'.
 * @returns A promise that resolves to a base64-encoded image data URL of the generated painting.
 */
export async function generatePainting(imageDataUrl: string, style: 'watercolor' | 'oil'): Promise<string> {
  ...
  const prompt = style === 'watercolor'
    ? `Create a traditional watercolor painting from the front of this building. Add a tiny signature that says "Gemini"`
    : `Create a traditional oil painting from the front of this building, with visible brush strokes and a rich, textured look. Add a tiny signature that says "Gemini"`;
